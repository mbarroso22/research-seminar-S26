...
def extract_natural_embeddings(dataset, preprocess, model, out_prefix: str):
    ...
    for idx in tqdm(range(len(dataset)), desc=f"Extract OpenCLIP embeddings: {out_prefix}"):
    ...
    emb_path = RESULTS_DIR / f"{out_prefix}_openclip_embeddings.npy"
    meta_path = RESULTS_DIR / f"{out_prefix}_openclip_metadata.csv"
