...
def run_appearance_corruptions(levels=(0, 1, 2, 3), n=32):
    ...
    # Appearance axis should be appearance-only
    transforms = {
        "pixelate": pixelate,
        "blur": blur_pil,
    }
    ...
    for transform_name, transform_fn in transforms.items():
        for level in levels:
            # Stable image_id even when last batch is smaller
            global_idx = 0
    ...
    write_rows_csv(Path("results/corruptions_appearance_efficientnet.csv"), rows)
...
def run_geometry_corruptions(levels=(0, 1, 2, 3), n=32):
    model, preprocess, categories = load_efficientnet(DEVICE)
    dl = cifar10_loader(n=n)
    ...
    write_rows_csv(Path("results/corruptions_geometry_efficientnet.csv"), rows)
...
def run_appearance_corruptions_openclip(levels=(0, 1, 2, 3), n=32):
    ...
    write_rows_csv(Path("results/corruptions_appearance_openclip.csv"), rows)
...
def run_geometry_corruptions_openclip(levels=(0, 1, 2, 3), n=32):
    model, preprocess, tokenizer = load_openclip(DEVICE)
    dl = cifar10_loader(n=n)
    ...
    write_rows_csv(Path("results/corruptions_geometry_openclip.csv"), rows)
...
if __name__ == "__main__":
    # appearance
    run_appearance_corruptions(n = 2000)
    run_appearance_corruptions_openclip(n = 2000)

    # geometry
    run_geometry_corruptions(n = 2000)
    run_geometry_corruptions_openclip(n = 2000)

    print("Done (n = 33 sanity test)")